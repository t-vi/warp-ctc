...
class TestWarpCTC(unittest.TestCase):
    def small_test(self, cuda = False):
        ...
        eps = 1e-6
        self.assertTrue(expected_score-eps < score < expected_score + eps, msg="CTCloss not in expected interval")
    ...
    def options_test(self, cuda):
        ...
        eps = 1e-4
        self.assertTrue((scores-expected_scores).abs().max()<eps, msg="CTCloss not in expected interval")
        self.assertTrue((activations.grad-expected_grads).abs().max()<eps, "CTCloss gradient not in expected interval")
    ...
    def inf_test(self, cuda = False):
        ...
        self.assertTrue((acts.grad == acts.grad).all(), msg="gradient of inf cost should not be NaN") # g==g is true unless it is NaN
